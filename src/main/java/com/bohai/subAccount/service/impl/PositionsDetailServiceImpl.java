...
public class PositionsDetailServiceImpl implements PositionsDetailService {
	...
	public long doFindPositionsDetail(String Subuserid, String Combokey, String Direction, String Instrumentid,
		...
		for (PositionsDetail positionsDetail : listPositionsDetail) {
			if (positionsDetail.getVolume() >= retVolume) {
				...
				BuyDetail buyDetail = new BuyDetail();
				buyDetail.setCombokey(positionsDetail.getCombokey());
				buyDetail.setSellcombokey(Combokey+"@"+positionsDetail.getVolume());
	...
	public List<PositionsDetail2> findGroupByPositionsDetail() throws FutureException{
		logger.info("findGroupByPositionsDetail");
		List<PositionsDetail2> list = null;
		
		try {
			list = positionsDetailMapper.findGroupByPositionsDetail();
		} catch (Exception e) {
			logger.error("查询findGroupByPositionsDetail失败",e);
			throw new FutureException("","查询findGroupByPositionsDetail失败");
		}
