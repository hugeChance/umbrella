...
import com.bohai.subAccount.swt.riskback.helper.RiskMainMarketReceiveThread;
import com.bohai.subAccount.swt.riskback.helper.RiskMainTradeReceiveThread;
...
//        });
                ...
                if(list != null && list.size() > 0){
                    ...
                    for(InvestorPosition position : list){
                        ...
                        try {
                            ...
                            //上海合约
                            if(CommonUtil.checkSHPosition(instrumentid)){
                                Long yposition = position.getYdposition()==null?0:position.getYdposition();//昨仓
                                Long tposition = position.getPosition() - yposition;//今仓
                                if(yposition >0){
                                    //平昨仓
                                    inputOrderField.setCombOffsetFlag("4");
                                    inputOrderField.setVolumeTotalOriginal(yposition.intValue());
                                    this.fkqp(userName, inputOrderField);
                                }
                                if(tposition >0){
                                    //平今仓
                                    inputOrderField.setCombOffsetFlag("3");
                                    inputOrderField.setVolumeTotalOriginal(tposition.intValue());
                                    this.fkqp(userName, inputOrderField);
                                }
     ...
     * @param requestBody
     */
    public void fkqp(String userName,CThostFtdcInputOrderField inputOrderField){
        ...
        PrintWriter out;
        try {
            out = new PrintWriter(new OutputStreamWriter(this.tradeSocket.getOutputStream(),"UTF-8"));
